...
namespace _3D_3ngine_V1
{
    public class Rasterization
    {
        ...
        //   public Rasterization(Size size, List<float[]> vertices, List<int[]> triangles, float translateX, float translateY, float translateZ, float z)
        ...
        private void Init(Size size, float z, Vertex vert, Vertex translate, Instance[] instances)
        {
            ...
            instances[0].position = translate;
            


            /*
            Mtx rotate = Mtx.Rotate(vert);
            Mtx translate = Mtx.MakeTranslationMatrix(traslate);
            Mtx scale = Mtx.MakeScalingMatrix(z);

            inst1.transform = translate * rotate * scale; //Makes the tranformation insatnce where rotation, scaling and transformation works fine
            inst2.transform = translate * rotate * scale; //Makes the tranformation insatnce where rotation, scaling and transformation works fine
            */

            // Instance insta = new Instance(cube, new Vertex(3f, -2f, 30f), Mtx.Rotate(vert), z,Mtx.MakeTranslationMatrix(translate));
            //instances[0] = insta;
        ...
        public void Render(Instance[] instances)
        {
            
           
            /*
            Mtx rotate = Mtx.Rotate(vert);
            instances.orientation = rotate;
            Mtx translate = Mtx.MakeTranslationMatrix(translateVertex); 
            instances.transform = translate; */
        ...
        void DrawFilledTriangle(Vertex p0, Vertex p1, Vertex p2, Color color)
        {
            //var xLeft;
            //var xRight;
           ...
           /* var y0 = (int)p0.y;
            ...
            if (y1 < y0) { Swap(ref p1, ref p0); }
            else if (y2 < y0) { Swap(ref p2, ref p0); }
            else if (y2 < y1) { Swap(ref p2, ref p1); } */
        ...
        void DrawLine(Vertex p0, Vertex p1, Color color) //This function is in slide 16 of presentation "La linea"
        {
            var dx = p1.x - p0.x;
            var dy = p1.y - p0.y;
        ...
        {
            //DrawFilledTriangle(projected[triangle.v0], projected[triangle.v1], projected[triangle.v2], Color.Black);
        ...
        public void RenderScene(Camera camera, Instance[] instances)
        {
            ...
            Mtx transform;
            Model clipped;
           // instances.transform = traslate * rotate * scale; //Makes the tranformation insatnce where rotation, scaling and transformation works fine
